...
## Profession progression and building icons

Fishing, mining and foraging now each have three tool ranks, purchasable at profession levels 2/5/8 for 20/50/80 coins and 8/12/16 basic materials. Each rank adds one material per trip. Gather retains 8-resource/25-XP rules; Explore costs 12 resource for 35 XP and lower base yield, advancing a guaranteed three-exploration discovery at that site. Nine named discoveries across the three professions grant one-time 15/30/45-coin rewards by site tier. Completing a profession's three discoveries grants a free tool rank, capped at three. Existing story/skill gates and 999 material caps remain; saved flags preserve upgrades and collections. UI shows next tool milestone, costs and expandable collection progress. No timed clicking or rare-drop lottery.

Guild Hall and Dungeon Lodge capital buttons now use a small shared SVG building sheet instead of font-dependent chess/sword symbols. Validation: JavaScript syntax and full Maven package with 60 tests passed, including discovery reward repeat prevention and atomic tool costs/yield. No full browser visual pass in this small batch.

## Latest: rival encounters and company acceptance fix
Four optional Cedric practice duels now unlock after Hollow Reach, Deep Shaft, Sunken Hall and The Scarred Path, with each requiring the preceding duel victory. Find them at the capital Noticeboard & practice yard. Dialogue follows his shift from rival to ally. Wins pay 20/40/60/80 coins once; no loot or XP farming. Health, resource and potions are restored after victory, loss or retreat, with no death penalty or campaign progress changes. Active duels survive save/reload.

Fixed Adventure/main-quest rendering after accepting the frontier company commission: the browser treated the route name string as a function, crashing rendering in subsequent story stages. Existing saves need no migration or reset. A focused browser-function regression checks briefing, acceptance, adventure, report and completion.

Validation: Maven package and Java tests passed, including three rival persistence/reward/gating checks; JavaScript syntax and story panel regression passed. No lengthy balance simulations. Push rework and wait for a successful Render deployment, then refresh the website. Remaining: safe per-fight auto-attack, richer NPC/world events and story branches, friendly player duels, optional late-game challenges and further balance from real play. Casino remains deferred.

## Playthrough and building icon follow-up
Guild Hall and Dungeon Lodge icons now use inline vector paths in their buttons, removing external SVG symbol loading. Visually confirmed both icons in the capital at a narrow desktop viewport.

Validation: all four fresh classes completed all 12 routes to level 60 with the existing deterministic strategy (155/132/204/132 combat turns for Warrior/Mage/Cleric/Rogue, no deaths). This is functional progression coverage, not proof of difficulty balance. Campaign audit now accepts the opening commission and fails if the campaign stops early. Two-player story HTTP playthrough passed route gating, pending-turn server restart, objective completion, separate saved progression, duplicate-reward rejection and build/loadout persistence. Browser checks passed character creation, briefing, company acceptance, starting Adventure, returning to town and accepting Mara's request; delivery correctly remains disabled without materials. Package/63 Java tests and story-panel regression passed. Live deployment still requires pushing rework.

## Shared chapter, level sync and dungeon contracts
- First chapter co-op presentation now has nine authored stage scenes across Old Timberline, Hollow Reach and Blighted Grove, with an explicit chapter-one conclusion and next-route guidance. Later chapters use existing route introductions/endings and generic middle-stage context. Scenes derive from persisted state and survive reconnects.
- Shared screens emphasize current stage and useful next actions. Guidance changes when supplies are in danger, objective work finishes or the enemy falls. Battle history starts collapsed. Finished parties can return to the capital directly.
- Main quests link to shared story contracts. Active-party banners explain paused solo actions and provide a return button. Solo command buttons are disabled while the server reports an active party; terminal party refresh updates the saved character view.
- Removed the three-level joining restriction. Both players still need the selected story route/contract unlocked. On departure, stronger characters sync to the lower character's level, capped at the selected route/contract ceiling. Enemies retain the mission minimum, so entering underprepared remains dangerous.
- Downscaled characters use proportional allocated attributes, reduced gear/legacy weapon bonuses and their base class abilities appropriate to the synced level. Specialisation/training/loadout and gear special effects are temporarily suppressed when downscaled. Original character data is preserved; normal builds return afterward. Health/resource percentages carry back, including zero health on defeat; consumed potions remain consumed. Existing active parties with no sync field retain their prior behavior.
- Three optional single-boss contracts: Rootbound Gate (levels 1–4); Sealed Forge (16–25, requires Blighted Grove, Ironbound Chieftain stance/heavy mechanics); Lantern Crypt (31–40, requires The Fracture, Lantern Lich interruptible recovery/heavy mechanics). New contracts award regional gear and bounded XP/coins, not regional story clears or legendary boss materials. They reuse existing boss mechanics and artwork, not new multi-room dungeon layouts.
- Verified: 69 Java tests; mixed-level new-contract victories; level-60/level-1 admission and save restoration; contract gates; shared-story HTTP run with pending-turn restart, progression and duplicate-reward protection; JS syntax/story-panel checks; browser review of active-party banner and first mission scene. No production saves changed. This is an initial scaling pass, not comprehensive economy or every class-pair balance validation.

Next focused work: actual two-player feedback on pacing, more distinct dungeon rooms/events and bespoke boss art, expand later chapter scenes, richer NPC interactions, safe per-fight auto-attack, then friendly duels and optional late-game challenges. Casino and additional kingdoms remain deferred. Push rework and confirm Render deployment before treating this batch as live.
