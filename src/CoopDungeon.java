...
public class CoopDungeon {
    ...
    public List<String> lastRoundEvents=new ArrayList<>();
    ...
    private void resolve(long now,Random random){
        ...
        for(int i=0;i<2;i++){Member m=members.get(i);Player p=players[i];String action=m.action;int before=enemy.getHealth();m.cooldowns.replaceAll((id,n)->Math.max(0,n-1));List<String> effects=new ArrayList<>();CombatAction kind=CombatAction.ATTACK;
            ...
            if(action.startsWith("ability:")&&before>enemy.getHealth())damagingAbility=true;
        ...
        if(!enemy.isDead()){
            List<String> counters=new ArrayList<>();boolean interrupted=BossCounters.interrupt(enemy,planned,damagingAbility,enemyHealth-enemy.getHealth(),counters);for(String line:counters)say(line);
            if(interrupted){}
            else if(enemy.hasStatus(StatusEffect.Kind.STUN))say(enemyName()+" is stunned; its attack is interrupted.");
                ...
                if(storyRoute!=null&&objective()==MissionObjective.INVESTIGATE&&"OBJECTIVE".equals(members.get(target).action))raw=(int)Math.round(raw*1.25);int recipient=protector[target]>=0?protector[target]:target;int damage=players[recipient].receiveDamage(raw,DamageType.PHYSICAL);say(enemyName()+" hits "+players[recipient].getName()+" for "+damage+".");}}
            enemy.endTurn();
        }
    ...
    public void say(String line){lastRoundEvents.add(line);while(lastRoundEvents.size()>30)lastRoundEvents.remove(0);log.add(line);while(log.size()>25)log.remove(0);}
