...
public class CoopDungeon {
    ...
    private void prepareStage(){enemyMaxHealth=stage==2?150+22*(level-1):90+15*(level-1);enemyHealth=enemyMaxHealth;enemyStatuses=new ArrayList<>();objectiveProgress=0;say("Stage "+(stage+1)+"/3: "+enemyName()+". "+objective().help);}
    ...
    public void start(String user,long now){require(user.equals(leader),"Only the host can start.");require(state.equals("LOBBY")&&members.size()==2,"Two players are needed.");level=members.stream().mapToInt(m->m.original.player().level()).max().orElse(1);state="BATTLE";deadline=now+60000;
    ...
    public String intent(){if(enemyHealth==0)return "Enemy defeated — complete the objective.";String target=members.size()<2?"your party":members.get(round%2).original.player().name();return round%3==2?"Heavy strike against "+target+" — defend or have your partner protect you.":round%3==1?"Root sweep hits both players.":"Strike against "+target+".";}
    ...
    private void resolve(long now,Random random){
        ...
        Enemy enemy=new Enemy(enemyName(),level,enemyMaxHealth,16+3*(level-1),5+level,0,0,0,0);enemy.restoreHealth(enemyHealth);enemy.restoreStatuses(enemyStatuses);
