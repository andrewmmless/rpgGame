...
public class CoopController {
    ...
    @PostMapping("/create") public Map<String,Object> create(Principal principal){String user=principal.getName();return tx.execute(status->{GameSave s=eligible(user);CoopDungeon p=new CoopDungeon();p.add(user,s);String id=UUID.randomUUID().toString().replace("-","").substring(0,16);jdbc.update("INSERT INTO hearthglen.rpg_coop(id,active,payload) VALUES (?,TRUE,?)",id,json.writeValueAsString(p));jdbc.update("INSERT INTO hearthglen.rpg_coop_members(username,party_id) VALUES (?,?)",user,id);return view(id,p,user);});}
    @PostMapping("/join") public Map<String,Object> join(Principal principal,@RequestBody Join body){String user=principal.getName();String id=body.code()==null?"":body.code().strip().toLowerCase(Locale.ROOT);CoopDungeon.require(id.matches("[a-f0-9]{16}"),"Enter the 16-character party code.");return tx.execute(status->{CoopDungeon p=party(id,true);p.add(user,eligible(user));jdbc.update("INSERT INTO hearthglen.rpg_coop_members(username,party_id) VALUES (?,?)",user,id);save(id,p);return view(id,p,user);});}
    ...
    private void finish(CoopDungeon party){
        ...
        for(CoopDungeon.Member member:party.members.stream().sorted(Comparator.comparing(m->m.username)).toList()){
            ...
            Equipment reward=won?Equipment.drop(new Random(),member.original.player().level(),true):null;
    ...
    private Map<String,Object> view(String id,CoopDungeon p,String user){Map<String,Object> out=new LinkedHashMap<>();out.put("joined",true);out.put("id",id);out.put("state",p.state);out.put("host",user.equals(p.leader));out.put("round",p.round);out.put("deadline",p.deadline);out.put("serverTime",System.currentTimeMillis());out.put("log",p.log);out.put("enemy",Map.of("name","Rootbound Warden","health",p.enemyHealth,"maxHealth",p.enemyMaxHealth,"intent",p.intent()));
