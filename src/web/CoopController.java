...
public class CoopController {
    ...
    public record Create(Integer route,Long generation,String dungeon) {}
    public record Join(String code,Long generation) {} public record Move(String id,int round,String action,String value) {}
    ...
    @PostMapping("/create") public Map<String,Object> create(Principal principal,@RequestBody(required=false) Create body){
        for(int attempt=0;attempt<5;attempt++){try{return createParty(principal.getName(),body);}catch(org.springframework.dao.DuplicateKeyException collision){/* Retry with a fresh transaction and code. */}}
        throw new IllegalArgumentException("Could not allocate a party code. Please try again.");
    }
    private Map<String,Object> createParty(String user,Create body){return tx.execute(status->{SocialStore.lock(jdbc);CharacterSlots.validate(jdbc,user,body==null||body.generation()==null?0:body.generation());GameSave s=eligible(user);CoopDungeon p=new CoopDungeon();if(body!=null&&body.route()!=null)p.selectStory(body.route());else if(body!=null)p.contractId=DungeonContract.get(body.dungeon()).id();p.add(user,s);String id=inviteCode();jdbc.update("INSERT INTO hearthglen.rpg_coop(id,active,payload) VALUES (?,TRUE,?)",id,json.writeValueAsString(p));jdbc.update("INSERT INTO hearthglen.rpg_coop_members(username,party_id) VALUES (?,?)",user,id);return view(id,p,user);});}
    @PostMapping("/join") public Map<String,Object> join(Principal principal,@RequestBody Join body){String user=principal.getName();String id=body.code()==null?"":body.code().strip().toLowerCase(Locale.ROOT);joinLimit(user);CoopDungeon.require(id.matches("[a-z2-9]{6}|[a-f0-9]{16}"),"Enter the six-character party code (older codes also work).");return tx.execute(status->{SocialStore.lock(jdbc);CharacterSlots.validate(jdbc,user,body.generation()==null?0:body.generation());CoopDungeon p=party(id,true);p.add(user,eligible(user));jdbc.update("INSERT INTO hearthglen.rpg_coop_members(username,party_id) VALUES (?,?)",user,id);save(id,p);return view(id,p,user);});}
    @PostMapping("/move") public Map<String,Object> move(Principal principal,@RequestBody Move body){String user=principal.getName();return tx.execute(status->{SocialStore.lock(jdbc);String id=membership(user);CoopDungeon.require(id!=null&&id.equals(body.id()),"This is not your current party.");CoopDungeon p=party(id,true);CoopDungeon.require(p.members.stream().anyMatch(m->m.username.equals(user)),"Not a party member.");String before=p.state;
        switch(body.action()==null?"":body.action()){
            case "approach" -> p.chooseApproach(user,body.value());
    ...
    private void finish(CoopDungeon party){
        ...
        for(CoopDungeon.Member member:party.members.stream().sorted(Comparator.comparing(m->m.username)).toList()){
            ...
            SubArea route=party.storyRoute==null?null:SubArea.get(party.storyRoute);
            int itemLevel=route==null?Math.max(party.contract().minLevel(),Math.min(party.contract().maxLevel(),party.level)):Math.max(route.minLevel(),Math.min(route.maxLevel(),member.original.player().level()));
            Equipment reward=won?Equipment.regionalDrop(new Random(),itemLevel,true,member.original.player().type(),route==null?party.contract().area():route.area()):null;
            int used=Math.min(3,member.original.player().potions())-member.potions;
            if(won&&social.infirmary(party)){Player healed=member.player();healed.heal(Math.max(1,healed.getMaxHealth()*15/100));member.store(healed);party.say("The guild infirmary tends "+healed.getName()+" after victory.");}
            game.returnFromCoop(member.restoredHealth(),member.restoredResource(),used,party.xpReward(),party.coinReward(),reward,won);
            int bonus=won&&route!=null?game.completeCoopStory(route.index()):0;
            if(won)party.rewards.add(new CoopDungeon.Reward(member.original.player().name(),party.xpReward()+bonus,party.coinReward(),reward,stored.inventory().size()>=30?reward.value():0));
    ...
    private Map<String,Object> view(String id,CoopDungeon p,String user){Map<String,Object> out=new LinkedHashMap<>();out.put("missionName",p.missionName());out.put("scene",SharedMissionScene.view(p));out.put("lastRoundEvents",p.lastRoundEvents);out.put("trophy",p.state.equals("VICTORY")&&p.storyRoute!=null&&p.storyRoute%3==2?BossMaterials.name(p.storyRoute/3):"");out.put("storyChoice",p.members.isEmpty()?"":StoryConsequences.choice(p.members.get(0).original.claimed()));out.put("approach",p.approach);out.put("story",p.storyRoute!=null);out.put("stage",p.stage+1);
        if(p.storyRoute!=null)out.put("objective",Map.of("title",p.objective().title,"action",p.objective().action,"help",p.objective().help,"progress",p.objectiveProgress,"target",3,"integrity",p.integrity,"escort",p.objective()==MissionObjective.ESCORT));
        out.put("comboCooldown",Math.max(0,p.comboReadyRound-p.round));out.put("joined",true);out.put("id",id);out.put("state",p.state);out.put("host",user.equals(p.leader));out.put("round",p.round);out.put("rescuesUsed",p.rescuesUsed);out.put("duoUnlocked",p.duoUnlocked);out.put("duoUsed",p.duoUsed);out.put("deadline",p.deadline);out.put("serverTime",System.currentTimeMillis());out.put("log",p.log);out.put("rewards",p.rewards);out.put("enemy",Map.of("name",p.enemyName(),"health",p.enemyHealth,"maxHealth",p.enemyMaxHealth,"intent",p.intent(),"phase",p.phase()));
        out.put("members",p.members.stream().map(m->{Player player=m.player();Map<String,Object> row=new LinkedHashMap<>();row.put("name",player.getName());row.put("type",player.getPlayerClass());row.put("level",player.getLevel());row.put("originalLevel",m.original.player().level());row.put("health",m.health);row.put("maxHealth",player.getMaxHealth());row.put("resource",m.resource);row.put("maxResource",player.getMaxResource());row.put("potions",m.potions);row.put("you",m.username.equals(user));row.put("ready",m.action!=null);row.put("action",m.action==null?"":m.action);row.put("abilities",player.getAvailableAbilities().stream().map(a->Map.of("id",a.id(),"name",a.name(),"cost",a.cost(),"cooldown",m.cooldowns.getOrDefault(a.id(),0))).toList());return row;}).toList());return out;}
}
