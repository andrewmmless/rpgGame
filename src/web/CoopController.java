...
public class CoopController {
    ...
    public record Create(Integer route,Long generation) {}
    ...
    private void finish(CoopDungeon party){
        ...
        for(CoopDungeon.Member member:party.members.stream().sorted(Comparator.comparing(m->m.username)).toList()){
            ...
            int itemLevel=route==null?Math.min(4,member.original.player().level()):Math.max(route.minLevel(),Math.min(route.maxLevel(),member.original.player().level()));
            Equipment reward=won?Equipment.regionalDrop(new Random(),itemLevel,true,member.original.player().type(),route==null?Area.WHISPERING_WOODS:route.area()):null;
    ...
    private Map<String,Object> view(String id,CoopDungeon p,String user){Map<String,Object> out=new LinkedHashMap<>();out.put("missionName",p.missionName());out.put("lastRoundEvents",p.lastRoundEvents);out.put("trophy",p.state.equals("VICTORY")&&p.storyRoute!=null&&p.storyRoute%3==2?BossMaterials.name(p.storyRoute/3):"");out.put("storyChoice",p.members.isEmpty()?"":StoryConsequences.choice(p.members.get(0).original.claimed()));out.put("approach",p.approach);out.put("story",p.storyRoute!=null);out.put("stage",p.stage+1);
