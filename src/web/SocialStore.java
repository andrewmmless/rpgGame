...
public class SocialStore {
    ...
    public void completed(CoopDungeon p){String a=p.members.get(0).username,b=p.members.get(1).username;Bond bond=bond(a,b);long now=System.currentTimeMillis();boolean credit=p.round>=3&&now-bond.lastCredit>=300000;
        if(credit){bond.clears++;bond.lastCredit=now;}String memory=java.time.LocalDate.now(java.time.ZoneOffset.UTC)+" · "+"Rootbound Gate"+" cleared in "+p.round+" rounds; "+p.rescuesUsed+" rescue used.";bond.memories.add(memory);while(bond.memories.size()>20)bond.memories.remove(0);saveBond(a,b,bond);
        String id=guildId(a);if(id!=null&&id.equals(guildId(b))){GuildHouse g=guild(id);if(credit)g.materials=Math.min(999,g.materials+1);g.owned.add("warden_trophy");if(bond.clears>=5)g.owned.add("bond_lantern");g.record(memory+(credit?" House supplies delivered.":" Bond credit is limited to one clear per five minutes."));saveGuild(id,g);}
